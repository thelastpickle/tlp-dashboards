...
.addRow(
  ...
  .addPanel(
    ...
    .addTarget(
      prometheus.target(
        'sum by (environment, cluster) (max by (environment, cluster, datacenter, rack, node) (changes(org_apache_cassandra_metrics_threadpools_value{scope="Native-Transport-Requests", environment="$environment", cluster="$cluster", datacenter=~"$datacenter", rack=~"$rack", node=~"$node"}[2m:30s])) > bool 0)',
        legendFormat='Nodes Coordinating Requests (Native protocol)',
      )
    )
    .addTarget(
      prometheus.target(
        'sum by (environment, cluster) (max by (environment, cluster, datacenter, rack, node) (changes(org_apache_cassandra_metrics_threadpools_value{scope="GossipStage", environment="$environment", cluster="$cluster", datacenter=~"$datacenter", rack=~"$rack", node=~"$node"}[2m:30s])) > bool 0)',
